...
module.exports = function() {
  ...
  const computeFromInterval = () => {
    ...
    const now = dateForTimezone(new Date()).valueOf();
    ...
    try {
      ...
      while (this.attrs.nextRunAt.valueOf() < now) {
        this.attrs.nextRunAt = cronTime._getNextDateFrom(this.attrs.nextRunAt);
        debug('[%s:%s] nextRunAt set to [%s]', this.attrs.name, this.attrs._id, this.attrs.nextRunAt.toISOString());
      }
    } catch (e) {
      ...
      try {
        if (!this.attrs.lastRunAt && humanInterval(interval)) {
          ...
          this.attrs.nextRunAt = new Date(0);
          this.attrs.nextRunAt.setUTCSeconds(Math.round((lastRun.valueOf() + humanInterval(interval)) / 1000));
          debug('[%s:%s] nextRunAt set to [%s]', this.attrs.name, this.attrs._id, this.attrs.nextRunAt.toISOString());
        }

        while (this.attrs.nextRunAt.valueOf() < now) {
          let next = new Date(0);
          next.setUTCSeconds(Math.round((this.attrs.nextRunAt.valueOf() + humanInterval(interval)) / 1000));
          this.attrs.nextRunAt = next;
          debug('[%s:%s] nextRunAt set to [%s]', this.attrs.name, this.attrs._id, this.attrs.nextRunAt.toISOString());
        }
      } catch (e) {}
    } finally {
